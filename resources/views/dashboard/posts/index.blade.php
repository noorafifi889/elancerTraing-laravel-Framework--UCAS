                            ...
                            <div class="flex items-start gap-4">
                                ...
                                <div class="flex-grow grid grid-cols-1 md:grid-cols-12 gap-4 items-center">
                                    ...
                                    <div class="md:col-span-2">
                                        @if ($post->trashed())
                                            <span
                                                class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-red-200 text-{{ $post->status->getColor() }}-700 text-[12px] font-bold border border-green-200">
                                                <span
                                        ...
                                        @else
                                            <span
                                                class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-green-50 text-{{ $post->status->getColor() }}-700 text-[12px] font-bold border border-green-200">
                                                <span
                                                    class="h-1.5 w-1.5 rounded-full bg-{{ $post->status->getColor() }}-600"></span>
...
@if ($post->trashed())
    ...
    <!-- Restore -->
    <button
        onclick="if(confirm('Restore this post?')) document.getElementById('restorepost-{{ $post->id }}').submit();"
        class="p-2 text-green-600 hover:bg-surface-container rounded-lg transition-all"
        title="Restore">
        <span class="material-symbols-outlined">refresh</span>
    </button>

    <form id="restorepost-{{ $post->id }}"
          method="POST"
    ...
    <!-- Force Delete (🔥 جديد) -->
    <button
        onclick="if(confirm('Permanently delete this post?')) document.getElementById('forceDelete-{{ $post->id }}').submit();"
        class="p-2 text-red-700 hover:bg-red-100 rounded-lg transition-all"
        title="Force Delete">
        <span class="material-symbols-outlined">delete_forever</span>
    </button>

    <form id="forceDelete-{{ $post->id }}"
          method="POST"
...
@else
    ...
    <!-- Soft Delete -->
    <button
        onclick="if(confirm('Delete this post?')) document.getElementById('deletepost-{{ $post->id }}').submit();"
        class="p-2 text-red-600 hover:bg-surface-container rounded-lg transition-all"
        title="Delete">
