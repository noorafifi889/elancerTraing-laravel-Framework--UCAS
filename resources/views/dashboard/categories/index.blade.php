    ...
    <main class="pt-24 pb-section-gap px-gutter max-w-container-max mx-auto">
        ...
        <header class="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
            ...
            <a href="{{ route('categories.create') }}"
                class="bg-primary text-white px-6 py-3 rounded-lg font-ui-button text-ui-button shadow-sm hover:opacity-90 active:scale-95 transition-all flex items-center gap-2 whitespace-nowrap">
        ...
        <!-- Session Alert -->
        @if(session('success'))
            <div class="bg-green-50 border border-green-200 text-green-700 p-4 rounded-lg mb-8 text-sm">
                {{ session('success') }}
            </div>
        @endif
        ...
        <div class="flex flex-col md:flex-row gap-4 items-center justify-between mb-8">
            <div class="relative w-full md:w-96">
                ...
                <input class="w-full bg-surface-container-lowest border border-outline-variant rounded-xl pl-12 pr-4 py-3 font-ui-label text-ui-label focus:border-primary outline-none transition-all"
        ...
        <div class="grid grid-cols-1 md:grid-cols-12 gap-6">
                ...
                <div class="relative z-10">
                    <div class="flex justify-between items-start mb-6">
                        ...
                        <div class="flex gap-2">
                            ...
                            <form action="{{ route('categories.destroy', $topCategory->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this category?');" class="inline">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="p-2 hover:bg-surface-variant rounded transition-colors hover:text-red-600 text-on-surface-variant" title="Delete">
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-8 mb-8">
                        ...
                        <div>
                            ...
                            <p class="font-headline-md text-2xl font-bold">{{ $topCategory->total_views ?? 0 }}</p>
                        </div>
                        <div>
            ...
            <!-- Remaining Categories (4 Columns) -->
                ...
                <div class="md:col-span-4 bg-surface-container-lowest border border-outline-variant rounded-xl p-8 hover:border-primary transition-colors flex flex-col justify-between group relative">
                    <div>
                        <div class="flex justify-between items-start mb-4">
                            <h2 class="font-headline-md text-2xl font-bold text-on-surface truncate pr-6">{{ $category->name }}</h2>
                            <div class="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity absolute top-8 right-6">
                                ...
                                <form action="{{ route('categories.destroy', $category->id) }}" method="POST" onsubmit="return confirm('Are you sure?');" class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="text-on-surface-variant hover:text-red-600 transition-colors bg-transparent border-none cursor-pointer">
            ...
            <!-- Add Category Skeleton Button -->
            <a href="{{ route('categories.create') }}"
                class="md:col-span-4 bg-surface-container-lowest border border-outline-variant rounded-xl p-8 border-dashed flex flex-col items-center justify-center text-center opacity-60 hover:opacity-100 hover:bg-surface-container-low transition-all cursor-pointer min-h-[200px]">
                    ...
                    <tbody class="divide-y divide-outline-variant">
                        @forelse($categories as $category)
                            <tr class="hover:bg-surface transition-colors">
                                ...
                                <td class="px-6 py-4">
                                    {{-- فحص الحالة بشكل مرن بناءً على الكنترولر المعلق حالياً --}}
                                    @if(!isset($category->status) || $category->status === 'active' || $category->status === 'published')
                                ...
                                <td class="px-6 py-4 font-ui-label text-ui-label text-on-surface">
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div class="flex justify-end gap-3">
                                        <a href="{{ route('categories.edit', $category->id) }}" class="text-on-surface-variant hover:text-primary transition-colors pt-1">
                                            <span class="material-symbols-outlined">edit</span>
                                        </a>
                                        
                                        <!-- تم الإصلاح: تحويل الرابط إلى Form حقيقي مخصص للحذف من قاعدة البيانات -->
                                        <form action="{{ route('categories.destroy', $category->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this category?');" class="inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="text-on-surface-variant hover:text-red-600 transition-colors bg-transparent border-none cursor-pointer pt-1">
                                                <span class="material-symbols-outlined">delete</span>
                                            </button>
                                        </form>
                                    </div>
